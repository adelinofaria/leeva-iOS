//
//  CampaignRemovalsViewController.m
//  Leeva
//
...
@implementation CampaignRemovalsViewController
...
- (void)loadView
{
    ...
    self.title = NSLocalizedString(@"campaign_details_removals", @"");
    ...
    column1.text = NSLocalizedString(@"campaign_removes_bar_column1", @"");
    ...
    column2.text = NSLocalizedString(@"campaign_removes_bar_column2", @"");
    ...
    labelTotalValue.text = _campaign.uniqueOpens;
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section
{
    ...
    if ([_campaign.unsubscribesArray count] > maxRows) {
        return maxRows;
    }
    else
        return [_campaign.unsubscribesArray count];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.textLabel.text = [((NSDictionary *)[_campaign.unsubscribesArray objectAtIndex:indexPath.row]) objectForKey:@"Email"];
    cell.detailTextLabel.text = [((NSDictionary *)[_campaign.unsubscribesArray objectAtIndex:indexPath.row]) objectForKey:@"Date"];
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    if ([_campaign.unsubscribesArray count] > maxRows && y > h) {
        maxRows = maxRows+25;
